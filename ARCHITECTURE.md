...
- `app-health-check.html` checks editor readiness and non-destructive app health diagnostics; `src/verify/app-health-frame-runtime.ts` owns iframe polling/helpers, and `src/verify/app-health-summary-runtime.ts` owns static health and DOM summary assertions used by that page
