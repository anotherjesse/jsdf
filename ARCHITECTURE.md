...
- `graph-check.html` checks graph inspector controls, graph editing, storage, dialogs, and history behavior; `src/verify/graph-history-runtime.ts` owns the graph history, change-journal, and numeric scrub verifier fixtures, `src/verify/source-patch-runtime.ts` owns source patch verifier fixtures, and `src/verify/source-workspace-runtime.ts` owns the source dialog and workspace-storage verifier slice used by that page
