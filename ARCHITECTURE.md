...
- `src/editor/source-language-features.ts` owns Monaco JavaScript language providers for API completions, formatting, hover docs, signature help, SDF quick fixes, graph source-link inlay hints, and inlay-hint command routing; `src/editor/source-editor-markers.ts` owns runtime marker projection and preferred quick-fix application inside a Monaco editor.
...
- `editor/clean-source-patch.ts` finds links between source ranges and graph node params; `editor/graph-source-patch-table.ts` owns the declarative node-field to source-call argument mapping used by that patcher.
...
- `app-health-check.html` checks editor readiness and non-destructive app health diagnostics; `src/verify/app-health-frame-runtime.ts` owns iframe polling/helpers, `src/verify/app-health-summary-runtime.ts` owns static health and DOM summary assertions, and `src/verify/app-health-shortcuts-runtime.ts` owns keyboard shortcut and focus verifier fixtures used by that page
- `editor-check.html` checks Monaco/source-link/editor integration behavior; `src/verify/editor-source-runtime.ts` owns source-link fixture helpers, navigation, hit-testing, inlay hints, tooltips, and selection restore checks, while `src/verify/editor-tooling-runtime.ts` owns the API hint, signature help, diagnostics, prettify, auto-return, preferences, and quick-fix verifier fixtures used by that page
- `graph-check.html` checks graph inspector controls, graph editing, storage, dialogs, and history behavior; `src/verify/graph-history-runtime.ts` owns the graph history, change-journal, and numeric scrub verifier fixtures, `src/verify/graph-orientation-runtime.ts` owns the orientation and custom matrix verifier fixture, `src/verify/graph-node-runtime.ts` owns shared verifier graph-node lookup helpers, `src/verify/source-patch-runtime.ts` owns source patch verifier fixtures, and `src/verify/source-workspace-runtime.ts` owns the source dialog and workspace-storage verifier slice used by that page
