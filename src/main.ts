...
import { sessionIdFromLocation } from "./editor/browser-session";
...
async function startApp(): Promise<void> {
  ...
  const initialSource = projectInitialState?.source ?? sourceForExample(activeExampleId);
  ...
  const projectSwitcher = createProjectSwitcher({
    ...
    elements: elements.projectSwitcher,
  ...
  const sourceWorkspaceSession = createSourceWorkspaceSession({
    ...
    draftStorageKey: browserSessionId ? draftStorageKey(browserSessionId) : undefined,
    currentSource: appState.currentSourceValue,
