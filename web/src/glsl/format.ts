...
export function f(value: number): string {
  if (!Number.isFinite(value)) throw new Error(`invalid GLSL number: ${value}`);
...
export function v2(value: readonly number[]): string {
  return `vec2(${f(value[0])}, ${f(value[1])})`;
}
...
export function mat2Mul(matrix: number[][], variable: string): string {
  return `vec2(dot(${variable}, vec2(${f(matrix[0][0])}, ${f(matrix[1][0])})), dot(${variable}, vec2(${f(matrix[0][1])}, ${f(matrix[1][1])})))`;
}

export function mat3Mul(matrix: number[][], variable: string): string {
  return `vec3(dot(${variable}, vec3(${f(matrix[0][0])}, ${f(matrix[1][0])}, ${f(matrix[2][0])})), dot(${variable}, vec3(${f(matrix[0][1])}, ${f(matrix[1][1])}, ${f(matrix[2][1])})), dot(${variable}, vec3(${f(matrix[0][2])}, ${f(matrix[1][2])}, ${f(matrix[2][2])})))`;
}
