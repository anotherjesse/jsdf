...
export function viewPanels(layout: PreviewLayout, width: number, height: number): ViewPanel[] {
  if (layout === "single") {
    return [{ x: 0, y: 0, width, height, direction: null }];
  }
  ...
  return [
    { x: 0, y: topY, width: leftWidth, height: topHeight, direction: null },
    { x: rightX, y: topY, width: rightWidth, height: topHeight, direction: [0, 0, 1] },
    { x: 0, y: 0, width: leftWidth, height: bottomHeight, direction: [1, 0, 0] },
    { x: rightX, y: 0, width: rightWidth, height: bottomHeight, direction: [0, 1, 0] },
  ];
}
